<template>
  <div class="tool">
    ...
    <myIcon type="language" class="icon tool-language"
      @click="$i18n.locale == 'cn' ? ($i18n.locale = 'en') : ($i18n.locale = 'cn')" />
    <a-icon type="skin" class="tool-skin icon" @click="showBackgroundModal = true" />
    <a href="https://mzc-chatdapptest.seaeye.cn/" target="_blank" class="tool-home icon">
      <a-icon type="home" />
    </a>
    ...
    <a-modal :title="$t('tool.theme')" :visible="showBackgroundModal" footer="" @cancel="showBackgroundModal = false">
      <div class="tool-user-info">
        ...
        <a-input v-model="backgroundInput" class="tool-user-input"
          :placeholder="$t('tool.please_enter_the_background_image_url')"></a-input>
        <a-button type="primary" @click="changeBackground(backgroundInput)">{{ $t('tool.confirm') }}</a-button>
      </div>
      <div class="tool-recommend">
        <div class="recommend" @click="changeBackground(BACKGROUND.URL)"
          v-for="(BACKGROUND, index) in COMMON.BACKGROUND_LIST" :key="index">
